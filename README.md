...
This wiki generator reads your code directly. It parses the repository into a symbol graph, then uses a local LLM to write pages that only cite what is present in the graph.

This project tests how much grounding can improve generated documentation.

Everything runs locally. There are no cloud calls or API keys. It connects to any OpenAI-compatible server you run, such as LM Studio, Ollama, or vLLM.

---

## 🤝 Contributing

Issues and pull requests are limited to collaborators. If you are not a collaborator, use Discussions to share feedback, suggestions, or bugs. Relevant input will be considered.

---

## ⚠️ Disclaimer

This documentation tool was built for personal use. The pages are generated by an LLM. Grounding reduces the gap between the documentation and the code, but does not eliminate it.
